...
class BaseModel(torch.nn.Module):
    ...
    def _predict_augment(self, x1, x2):
        """Perform augmentations on input image x and return augmented inference."""
        LOGGER.warning(
            f"{self.__class__.__name__} does not support 'augment=True' prediction. "
            f"Reverting to single-scale prediction."
        )
        return self._predict_once(x1, x2)

    def _profile_one_layer(self, m, x1, x2, dt):
        """
        Desactivate for now
    ...
    def loss(self, x1, x2, preds=None):
        ...
        preds = self.forward(x1["img"], x2["img"]) if preds is None else preds
        # return self.criterion(preds, x1, x2)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch_b=3, ch_w=3, nc=None, verbose=True):
        """
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x1, x2):
                ...
                if self.end2end:
                    return self.forward(x1, x2)["one2many"]
                return self.forward(x1, x2)[0] if isinstance(m, (Segment, YOLOESegment, Pose, OBB)) else self.forward(x1, x2)

            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch_b, s, s), torch.zeros(1, ch_w, s, s))])  # forward
...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg="yolo11n-seg.yaml", ch_b=3, ch_w=3, nc=None, verbose=True):
        """
