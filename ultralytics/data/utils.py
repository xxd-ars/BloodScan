...
def verify_image(args):
    ...
    try:
        ...
        if str(im_file).lower().endswith('.npy'):
            ...
            assert im.shape[2] == 6, f"Expected 6 channels, got {im.shape[2]} channels"
            assert im.dtype in [np.uint8, np.float32, np.float64], f"Unsupported dtype: {im.dtype}"
        else:
