...
#### 概述 ###
本模块负责通过Modbus TCP协议控制旋转和夹持设备
具体实现包括与多个从机(如机械手臂和夹持器)的通信
通过发送特定的十六进制数据指令, 实现设备的初始归零, 旋转和移动操作,
并通过线程事件机制协调旋转和拍照过程
...
3. **G_Send_HOM()**
   - **功能**：发送归零指令到G设备并等待反馈确认完成
   - **输入**：无
   - **输出**：打印归零完成信息

4. **R_Send_HOM()**
   - **功能**：发送归零指令到R设备并等待反馈确认完成
   - **输入**：无
   - **输出**：打印归零完成信息。

5. **G_Send_RC1()**
   - **功能**：发送RC1指令到G设备并等待反馈确认完成
   - **输入**：无
   - **输出**：打印RC1完成信息

6. **G_Send_RC2()**
   - **功能**：发送RC2指令到G设备并等待反馈确认完成
   - **输入**：无
   - **输出**：打印RC2完成信息。

7. **R_Send_RC1()**
   - **功能**：发送RC1指令到R设备并等待反馈确认完成
   - **输入**：无
   - **输出**：打印RC1完成信息并设置`photo_event`事件等待`rotate_event`事件
