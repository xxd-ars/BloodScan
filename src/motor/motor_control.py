...
#### 概述
本模块通过集成`m_ModBusTcp`和`m_RS485`两个子模块，提供了一套完整的电机初始化、定位、移动和检测操作的API接口。实现对电机的多轴控制（X、Y、Z）以及对设备的归零和半圆旋转操作。
...
22. **detect_once(X_Number)**
    - **功能**：执行一次完整的检测流程，包括Z轴、X轴、G设备和R设备的多次移动和旋转操作。
    - **输入**：`X_Number`（int）：X轴编号。
    - **输出**：无

23. **detect_all()**
    - **功能**：执行一次完整的批量检测流程，包括多次调用`detect_once`函数，并控制电机的定位和归零操作。
    - **输入**：无
    - **输出**：无
...
2. **电机定位**
   - 通过控制电机的多轴移动，将电机移动到指定位置。初始定位包括X轴左移55单位和Y轴前移2100单位。

3. **多轴移动**
   - 通过调用`m_RS485`模块的移动函数，控制X、Y、Z轴的移动。每个移动操作通过指定移动距离，实现电机的精确定位。

4. **设备旋转**
   - 通过调用`m_ModBusTcp`模块的旋转函数，控制R设备的半圆和四分之一圆的旋转操作。

5. **检测流程**
   - 通过调用`detect_once`函数，执行一次完整的检测流程，包括Z轴下降、X轴移动、G设备夹持和R设备旋转操作。通过线程事件机制`stop_event`实现流程的同步和等待操作。

6. **批量检测**
   - 通过调用`detect_all`函数，执行一次完整的批量检测流程。循环调用`detect_once`函数，并控制X轴和Y轴的移动，实现批量检测操作。

---

通过上述API接口和功能原理，本模块实现了对电机的多轴控制和检测操作，确保设备在自动化流程中的精确性和稳定性。'''
