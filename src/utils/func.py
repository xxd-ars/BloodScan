'''
完整的后端程序可以与PyQt5生成的界面文件ui_winqt一起使用
以实现复杂的图像处理和数据管理应用

logging configuration
配置了日志记录器以记录调试信息和错误
记录在app.log中线程变化的过程用于调试

threading
使用线程处理电机初始化 视频显示 图像捕获和数据添加
以确保界面不会因为这些操作而卡顿

video_show
使用OpenCV从摄像头捕获视频帧 并在QLabel上显示

timer_start, update_info
使用 QTimer 定时更新一些信息 如帧数和批次等

view_datasql, load_table_data
使用SQLite数据库操作存储和读取采样数据

条形码检测和K-means分层检测
使用zbar和K-means进行条形码识别和血液分层检测 并将结果显示在界面上

get_one_frame
get_one_frame里面的注释部分是真实环境下用于拍照测试的
未注释的是调用本地照片用于展示识别结果的

用户界面交互
界面元素的更新和事件处理 如开始检测 停止检测 继续检测等
detect_start
detect_stop
detect_continue
detect_end
...
import algorithm.zbar_v as zbar_v
import algorithm.K_means_5 as K_means_5
...
class MyMainWindow(QtWidgets.QMainWindow, ui_winqt.Ui_MainWindow):
    def __init__(self):
        ...
        self.capture = cv2.VideoCapture('v4l2src device=/dev/video0 ! image/jpeg, width=2048, height=1536, framerate=30/1 ! jpegdec ! videoconvert ! video/x-raw, format=BGR ! appsink', 
                                        cv2.CAP_GSTREAMER)
        ...
        self.frame_count = 0
        # Center align the QLabel
        # 方法会使控件内的内容如文本或图像在水平方向和垂直方向上都居中显示
        self.photoLB.setAlignment(Qt.AlignCenter)
    ...
    def update_info(self):
        ...
        self.collection_batch = self.capture.get(cv2.CAP_PROP_FPS)
    ...
    def get_one_frame(self):
        while True:
            # 调用photo_event的线程被等待 直到事件被触发set
            photo_event.wait()
